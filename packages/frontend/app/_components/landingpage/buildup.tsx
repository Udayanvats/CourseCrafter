...
const Buildup = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollPercent < 33.333333333333) {
        ...
        setProgressWidth1((scaleValue(scrollPercent, 0, 33.333333333333, 0, 100) / 100) * containerWidth);
        setProgressWidth2(0);
        setBreakpoint(0);
      } else if (scrollPercent >= 33.33 && scrollPercent < 66.66666666) {
        setProgressWidth1((scaleValue(33.333333333333, 0, 33.333333333333, 0, 100) / 100) * containerWidth);
        setProgressWidth2(((scaleValue(scrollPercent - 33.3333333, 0, 33.333333333333, 0, 100)) / 100) * containerWidth + 5);
        setBreakpoint(1);
      } else {
        setProgressWidth1((scaleValue(33.333333333333, 0, 33.333333333333, 0, 100) / 100) * containerWidth);
        setProgressWidth2(((scaleValue(66.66666666 - 33.3333333, 0, 33.333333333333, 0, 100)) / 100) * containerWidth + 5);
        setBreakpoint(2);
      }
    };
  ...
  return (
    <div className="row ">
      ...
      <div className="row h-[2350px]">

        <StickyBox offsetTop={0} offsetBottom={0} className="flex justify-center w-full bg-secondaryBackground h-screen justify-center flex items-center">
            ...
            <div className=" w-[90%]  ">
              ...
              <div className="px-16 mt-6" >
                <div className="flex justify-between items-center  text-2xl font-bold">
                  <span className={`text-5xl tracking-tighter ${breakpoint>=0&& "text-white"}`}>UPLOAD</span>
                  <div id="build" ref={containerRef} className="relative h-1 w-full bg-gray-500 rounded-full mx-4">
                    <div
                      className="absolute h-1 bg-white rounded-full progress-bar"
                      style={{ width: `${progressWidth1}px` }}
                    ></div>
                  </div>
                  <span className={`text-5xl tracking-tighter ${breakpoint>=1&& "text-white"}`}>CONVERT</span>
                  ...
                  <span className={`text-5xl tracking-tighter ${breakpoint>=2&& "text-white"}`}>STUDY</span>
                </div>
              </div>
